...
echo "========================================"
echo ""
echo "     H I L O P A N E L  INSTALLER"
...
# ================= INSTALL =================
install_panel() {
    echo -e "${GREEN}Cloning HILOPANEL...${RESET}"

    git clone $REPO

    # FIX: auto-detect folder (case-safe)
    ...
    # ================= CODE SANDBOX SAFE START =================
    PORT=${PORT:-3000}

    echo -e "${YELLOW}Starting React (CodeSandbox mode)...${RESET}"

    # Use dev server for React TS/Vite/CRA
    pm2 start npm --name "HILOPANEL" -- run dev -- --host 0.0.0.0 --port $PORT

    pm2 save

    echo -e "${GREEN}Panel running on port $PORT${RESET}"
    read -p "Press enter..."
}
