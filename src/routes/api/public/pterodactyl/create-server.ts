  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!env.pterodactyl?.pterodactyl_url || !env.pterodactyl?.pterodactyl_api_key) {
        ...
        return Response.json({ ok: true, pterodactyl_server_id: null, note: "Server record created locally. Configure nest/location to auto-provision." });
      },
    },
  },
});
